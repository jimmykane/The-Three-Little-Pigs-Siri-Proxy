...
if [ "${server2}" == "" ]
then
  server2="your.siri.proxy.server.com" # Doesn't matter if you don't have one for your server,
...
echo "Creating CA directory" >> $LOG_FILE
...
echo "Generating '${commonName}' CA request"
echo "Generating ${server1} & ${server2} certificate request" >> $LOG_FILE
echo "Generating '${commonName}' CA request" >> $LOG_FILE
echo "${countryName}" > $TMP_DIR/ca.args
echo "${stateOrProvinceName}" >> $TMP_DIR/ca.args
echo "${localityName}" >> $TMP_DIR/ca.args
...
echo "Please install ${SIRI_PROXY_SETTINGS}/ca.pem onto your phone!" >> $LOG_FILE
